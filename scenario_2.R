...
max_var= p
#all variable list
var.list = c(paste("X",1:max_var,sep=""))
#Some other list we will use
target.var.list = c("X1", "X2", "X3", "X4")
potconf.var.list = c("X1", "X2", "X3", "X4", "X5", "X6", "X7", "X8")
conf.var.list = c("X1", "X2")
...
for (i in 1:itr_max) {
  ...
  if (i != 1){
    ...
    df_sel.var.list.onet = read.csv("onet.sel.var.list.cor_0.0.csv")
    df_sel.var.list.onet = df_sel.var.list.onet [,-1]
    
    df_sel.var.list.adlas = read.csv("adlas.sel.var.list.cor_0.0.csv")
    df_sel.var.list.adlas = df_sel.var.list.adlas [,-1]
    
    df_sel.var.list.bort = read.csv("bort.sel.var.list.cor_0.0.csv")
    df_sel.var.list.bort = df_sel.var.list.bort [,-1]
    
    df_sel.var.list.bory = read.csv("bory.sel.var.list.cor_0.0.csv")
    df_sel.var.list.bory = df_sel.var.list.bory [,-1]
    
    df_sel.var.list.bacr = read.csv("bacr.sel.var.list.cor_0.0.csv")
    df_sel.var.list.bacr = df_sel.var.list.bacr [,-1]
    
    df_sel.var.list.bcee = read.csv("bcee.sel.var.list.cor_0.0.csv")
  ...
  #Creating treatment
  gA_x = 0.5*Data$X1 - Data$X2 + 0.3*Data$X5 - 0.3*Data$X6 + 0.3*Data$X7 - 0.3*Data$X8
  pA = expit( gA_x )
  Data$A = rbinom(length(pA), 1, pA) # simulate A 
  
  
  #Creating outcoem
  gY_xA =  Data$A + 2*Data$X1 + 2*Data$X2 + 5*Data$X3 + 5*Data$X4  
  Data$Y = gY_xA + rnorm(n=n,sd=sig_x) #adding random noise
  Data$Y = rnorm(n=n, mean = gY_xA, sd = 2)
  ...
  fit2=glmnet(as.matrix(xx), as.matrix(aa), alpha = alpha.opt, lambda = K.cv.fit$lambda.min, penalty.factor = pen, family="binomial")
  ...
  fit2=glmnet(as.matrix(xx),as.matrix(aa), alpha = 1, penalty.factor = pen, lambda = K.cv.fit$lambda.min, family="binomial")
  ...
  write.csv(df_att, "att.cor_0.0.csv")
...
boxplot(att.target, att.conf, att.potconf, att.adnet, att.adlas, att.Boruta_T, att.Boruta_Y, att.BACR, att.BCEE,  
        names = c("Targ", "Conf", "Pot.Conf", "ONet", "OLas", "Bor(T)", "bor(Y)", "BACR", "BCEE"), ylim = c(-0.2,2.3))
abline(h=bA, lty = 2)
dev.off()
